...
const sortData = [
  { label: 'Date: Old to new', id: 1, select: false },
  { label: 'Date: New to old', id: 2, select: true },
    ...
    <>
      <div className="flex lg:flex-nowrap justify-center items-center w-full min-h-[70vh]">
        <div className='w-full'>          
          ...
          <p className='text-extraa-purple-btn font-medium text-base md:text-lg md:font-extrabold px-12'>{startIndex} - {endIndex} of {productCount} Tickets(s)</p>
          ...
          />
            ...
            {allprod?.length > 0 ?( allprod?.map(({ id, brand,brand_logo,slug, name,metadata, type, category, images, product_variants, qty }) => (
              ...
              <EventCard name={name} brandLogo={brand_logo} slug={`/${slug}`} brand={brand} metadata={metadata} imageSrc={images} price={product_variants[0]?.sale_price} />
