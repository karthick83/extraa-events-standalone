...
// param.dealsId
const UserOrderDetails = ({ params,orderId }) => {
    const token = superadmin;
    // const orderId = decode(params?.orderChar) / 11111;
    // console.log(params)
    // console.log(orderId, "orderid");
    // console.log(decode(params?.orderChar), "decoded ")
                    ...
                    <div className="p-5  ">
                         {orderDetails &&
                            <div className="w-full  ">
                                ...
                                <div className="container max-w-[600px] mx-auto mt-8 ">
                                    ...
                                    <>
                                        ...
                                        <div className="flex w-full md:flex-row justify-between items-center bg-[#f1f1f1] rounded-t-3xl ">
                                            ...
                                            <div className=" my-4 ml-6 ">
                                                ...
                                                {orderDetails?.order_transactions?.length > 0 &&
                                                    <p className="text-sm">Transaction ID:
                                                        {orderDetails?.order_transactions?.map((t) => <span key={t?.id}> #{t?.id} </span>)}
                                                    </p>
                                                }
                                                {eventDetails &&
                                                    <>
                                                        <h3 className="text-l mt-4 font-bold"> Event Details</h3>
                                                        <p className="text-sm"><SfIconCalendarToday size="xs" /> {eventDetails && dayjs(eventDetails?.product_variant?.product?.metadata?.eventStartDate)?.format('ddd, D MMM YYYY')}</p>
                                                        <p className="text-sm"><SfIconSchedule size="xs" /> {eventDetails && dayjs(eventDetails?.product_variant?.product?.metadata?.eventStartDate)?.format('h:mm A')}</p>
                                            ...
                                            <img key={orderDetails?.order_products[0].id}
                                                src={orderDetails?.order_products[0]?.product_variant?.product?.images}
